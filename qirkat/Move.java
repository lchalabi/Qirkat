...
class Move {
     ...
     * may execute simultaneously.  Otherwise, the methods may attempt
     ...
     * of the operations are not "thread safe". Fortunately, we do not run
     * into this problem since there is no parallel programming going on.
     */
    ...
    static Move move(Move move0, Move move1) {
        ...
        if (move0.isVestigial()) {
            char col0m0 = move0._col0;
            char row0m0 = move0._row0;
            char col0m1 = move1._col0;
            char row0m1 = move1._row0;
            char col1m1 = move1._col1;
            char row1m1 = move1._row1;

            return move(col0m0, row0m0, col0m1, row0m1, move1);
        }
        if (move0.jumpTail() == null) {
            return move(move0._col0, move0._row0, move0._col1,
                    move0._row1, move1);
        } else {
            move0._nextJump._nextJump = move1;
            return move0;
        }
    }
     ...
     *  do something like if isjump then row is one less*/
    char jumpedRow() {
        if (!isJump()) {
            return row1();
        } else {
            int ind1 = index(col1(), row1());
            int ind0 = index(col0(), row0());
            char row1 = row(ind1);
            char row0 = row(ind0);
            if (row1 < row0) {
                int indjumped = ind1 + 5;
                return row(indjumped);
            } else if (row0 < row1) {
                int indjumped = ind1 - 5;
                return row(indjumped);
            } else {
    ...
    char jumpedCol() {
        int ind = index(col1(), row1());
        if (col0() - col1() > 1) {
            int indjumped = ind + 1;
            return col(indjumped);
        } else if (col0() - col1() < -1) {
            int indjumped = ind - 1;
            return col(indjumped);
        }
    ...
    private void toString(Formatter out) {
        Move pointer = this;
        if (pointer._nextJump == null) {
            out.format("%c%c-%c%c", pointer.col0(), pointer.row0(),
                    pointer.col1(), pointer.row1());
            return;
        } else {
            out.format("%c%c-%c%c", pointer.col0(), pointer.row0(),
                    pointer.col1(), pointer.row1());
            while (pointer.jumpTail() != null) {
                pointer = pointer._nextJump;
                out.format("-%c%c", pointer.col1(), pointer.row1());
            }
        }
    }
