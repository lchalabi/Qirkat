...
class Game {
    ...
    void process() {
        ...
        while (true) {
            ...
            if (_blackIsManual && _whiteIsManual) {
                white = new Manual(this, WHITE);
                black = new Manual(this, BLACK);
            } else if (_blackIsManual && !_whiteIsManual) {
                white = new AI(this, WHITE);
                black = new Manual(this, BLACK);
            } else if (_whiteIsManual && !_blackIsManual) {
                white = new Manual(this, WHITE);
                    ...
                    if (!_board.legalMove(move)) {
                        ...
                        if (_board.whoseMove() == WHITE) {
                            if (white instanceof AI) {
                                String msg = "White moves "
                                        + move.toString() + ".";
                                _reporter.outcomeMsg(msg);
                            }
                        } else {
                            if (black instanceof AI) {
                                String msg = "Black moves "
                                        + move.toString() + ".";
                                _reporter.outcomeMsg(msg);
                            }
                        }
    ...
    void doLoad(String[] operands) {
        try {
            FileReader freader = new FileReader(operands[0]);
            ReaderSource rsource = new ReaderSource(freader, false);
            _inputs.addSource(rsource);
    ...
    void doSet(String[] operands) {
        ...
        if (color.equals("black")) {
            PieceColor pcolorb = PieceColor.BLACK;
            _board.setPieces(operands[1], pcolorb);
        } else if (color.equals("white")) {
            PieceColor pcolorw = PieceColor.WHITE;
            _board.setPieces(operands[1], pcolorw);
        }
    }
